...
class kw_dict_mgr(object):
    ...
    def most_used(self):
        cmd = u'SELECT * FROM keyword_dict WHERE used_time = (SELECT MAX(used_time) FROM keyword_dict) ORDER BY id ASC;'
        result = self.sql_cmd(cmd)
        if len(result) > 0:
            return result
        else:
    ...
    def least_used(self):
        cmd = u'SELECT * FROM keyword_dict WHERE used_time = (SELECT MIN(used_time) FROM keyword_dict) ORDER BY id ASC;'
        result = self.sql_cmd(cmd)
        if len(result) > 0:
            return result
        else:
